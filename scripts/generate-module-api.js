...
async function main() {
  ...
  // 8. Generate Table template - columns dựa trên selected fields
  var columnsSnippet = [
    "    { title: 'ID', dataIndex: 'id', key: 'id', width: 80 },"
  ];
  selectedFields.forEach(function(f) {
    var label = f.name.charAt(0).toUpperCase() + f.name.slice(1);
    columnsSnippet.push("    { title: '" + label + "', dataIndex: '" + f.name + "', key: '" + f.name + "' },");
  });
  columnsSnippet.push([
    "    {",
    "      title: 'Thao tác',",
  ...
  var tableTemplate = [
    ...
    "        headerTitle={<Button type=\"primary\" onClick={handleCreate}>Thêm mới</Button>}",
    "        loading={loading}",
