 ...
 */
const baseEntityFields = ["id", "ma", "stt", "createdAt", "updatedAt", "deletedAt"];
 ...
 */
function mapTypeToTs(schemaProp) {
  if (!schemaProp) return "any";
  if (schemaProp.enum) {
    return schemaProp.enum.map(function(e) { return '"' + e + '"'; }).join(" | ");
  }
  ...
  switch (type) {
    ...
    case "boolean":
      return "boolean";
    case "array":
      return mapTypeToTs(schemaProp.items) + "[]";
    case "object":
...
function selectFieldsUI(fields) {
  return new Promise(function(resolve) {
    var currentIndex = 0;
    var selected = {};
    fields.forEach(function(f) { selected[f.name] = true; });
    ...
    function render() {
      ...
      fields.forEach(function(field, index) {
        ...
        console.log(cursor + " " + checkbox + " " + field.name + ": " + field.tsType + " " + reqStr);
      });
    }
    ...
    var onKeypress = function(str, key) {
      if (key.name === "up") {
        ...
        rl.close();
        var resultFields = fields.filter(function(f) { return selected[f.name]; });
...
function updateRoutes(routesFilePath, moduleNameLower) {
  ...
  if (content.includes(checkSingle) || content.includes(checkDouble)) {
    console.log("Route /" + moduleNameLower + " đã tồn tại trong routes.ts, bỏ qua.");
    return;
  }
  ...
  if (rIdx !== -1) {
    ...
    if (insertIdx !== -1) {
      var updated = content.slice(0, insertIdx) + "\n" + newRoute + content.slice(insertIdx);
...
async function main() {
  ...
  var moduleNameCamel = toCamel(rawName).charAt(0).toLowerCase() + toCamel(rawName).slice(1);
  var moduleNameUpper = toCamel(rawName).charAt(0).toUpperCase() + toCamel(rawName).slice(1);
  ...
  if (!schema) {
    console.error("❌ Không tìm thấy schema '" + entityName + "' trong API Document!");
    console.log("Các schema hiện có:", Object.keys(apiData.components.schemas).join(", "));
    process.exit(1);
  }
  ...
  for (var key in properties) {
    if (baseEntityFields.indexOf(key) === -1) {
      availableFields.push({
        name: key,
        tsType: mapTypeToTs(properties[key]),
        required: requiredList.indexOf(key) !== -1,
        nullable: properties[key].nullable === true
      });
    }
  }

  if (availableFields.length === 0) {
    console.warn("⚠️ Schema " + entityName + " không có field nào ngoài BaseEntity!");
  }
  ...
  var dirs = [moduleDir, path.join(moduleDir, "types"), path.join(moduleDir, "service"), path.join(moduleDir, "hooks"), pagesDir, pagesComponentsDir];
  dirs.forEach(function(dir) {
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  });

  // 5. Generate types template
  var typesFieldsArr = [];
  selectedFields.forEach(function(f) {
    var isOptional = (!f.required || f.nullable) ? "?" : "";
    typesFieldsArr.push("  " + f.name + isOptional + ": " + f.tsType + ";");
  });
  ...
  var hooksTemplate = [
    ...
    "  return useRequest(() => " + moduleNameCamel + "Service.getPage(options), { ...queryOptions });",
    "};",
    "",
    "export const useGet" + moduleNameUpper + "List = (options?: any, queryOptions?: any) => {",
    "  return useRequest(() => " + moduleNameCamel + "Service.getMany(options), { ...queryOptions });",
    "};",
    "",
    "export const useCount" + moduleNameUpper + " = (options?: any, queryOptions?: any) => {",
    "  return useRequest(() => " + moduleNameCamel + "Service.count(options), { ...queryOptions });",
    "};",
    "",
    "export const useGet" + moduleNameUpper + "One = (options?: any, queryOptions?: any) => {",
    "  return useRequest(() => " + moduleNameCamel + "Service.getOne(options), { ...queryOptions });",
    "};",
    "",
    "export const useExists" + moduleNameUpper + " = (options?: any, queryOptions?: any) => {",
    "  return useRequest(() => " + moduleNameCamel + "Service.exists(options), { ...queryOptions });",
    "};",
    "",
    "export const useGet" + moduleNameUpper + 'ById = (id: number | undefined, queryOptions?: any) => {',
    "  return useRequest(() => " + moduleNameCamel + "Service.findById(id!), { ready: !!id, ...queryOptions });",
    "};",
    "",
    "export const useGet" + moduleNameUpper + 'ByMa = (ma: string | undefined, queryOptions?: any) => {',
    "  return useRequest(() => " + moduleNameCamel + "Service.findByMa(ma!), { ready: !!ma, ...queryOptions });",
    "};",
    "",
    "export const useCreate" + moduleNameUpper + " = () => {",
    "  return useRequest((data: Create" + moduleNameUpper + "Dto) => " + moduleNameCamel + "Service.create(data), { manual: true });",
    "};",
    "",
    "export const useUpdate" + moduleNameUpper + " = () => {",
    "  return useRequest(({ id, data }: { id: number; data: Update" + moduleNameUpper + "Dto }) => " + moduleNameCamel + "Service.update(id, data), { manual: true });",
    "};",
    "",
    "export const useUpdateMany" + moduleNameUpper + " = () => {",
    "  return useRequest(({ filter, data }: { filter: Record<string, unknown>; data: Partial<" + moduleNameUpper + "> }) => " + moduleNameCamel + "Service.updateMany(filter, data), { manual: true });",
    "};",
    "",
    ...
    "  return useRequest((filter: Record<string, unknown>) => " + moduleNameCamel + "Service.deleteMany(filter), { manual: true });",
    "};",
    "",
    "export const useSoftDelete" + moduleNameUpper + " = () => {",
    "  return useRequest((filter: Record<string, unknown>) => " + moduleNameCamel + "Service.softDelete(filter), { manual: true });",
    "};",
    "",
    "export const useRestore" + moduleNameUpper + " = () => {",
    "  return useRequest((filter: Record<string, unknown>) => " + moduleNameCamel + "Service.restore(filter), { manual: true });",
    "};"
  ].join("\n");
  ...
  columnsSnippet.push("    { title: 'Mã', dataIndex: 'ma', key: 'ma', width: 120, sorter: true },");
  selectedFields.forEach(function(f) {
    var label = f.name.charAt(0).toUpperCase() + f.name.slice(1);
    columnsSnippet.push("    { title: '" + label + "', dataIndex: '" + f.name + "', key: '" + f.name + "', sorter: true },");
  });
  columnsSnippet.push([
    "    {",
    "      title: 'Thao tác',",
  ...
  var tableTemplate = [
    ...
    '} from "@/modules/' + moduleNameLower + '/hooks/' + moduleNameLower + '.hooks";',
    "import type { " + moduleNameUpper + " } from \"@/modules/" + moduleNameLower + "/types/" + moduleNameLower + ".types\";",
    "",
    ...
    "      if (value) {",
    '        query.like(key as keyof ' + moduleNameUpper + ', value as string);',
    "      }",
    "    });",
    ...
    "    pagination: any,",
    "    filters: any,",
    "    sorter: SorterResult<" + moduleNameUpper + "> | SorterResult<" + moduleNameUpper + ">[]",
    "  ) => {",
    ...
    '      <BaseModal.Form',
    "        open={formOpen}",
    '        title={isEdit ? "Sửa ' + moduleNameUpper + '" : "Thêm mới ' + moduleNameUpper + '"}',
    "        onCancel={closeForm}",
    "        onSubmit={() => formRef.current?.submit()}",
    "        loading={false}",
    '        submitText={isEdit ? "Cập nhật" : "Thêm mới"}',
    "      >",
    "        <" + moduleNameUpper + "Form ref={formRef} onSuccess={handleFormSuccess} />",
    "      </BaseModal.Form>",
    "",
    '      <BaseModal.Confirm',
  ...
  selectedFields.forEach(function(f) {
    var label = f.name.charAt(0).toUpperCase() + f.name.slice(1);
    var isRequired = f.required ? "rules={[{ required: true, message: 'Vui lòng nhập " + label + "' }]}" : "";
  ...
  var formTemplate = [
    ...
    "  Create" + moduleNameUpper + "Dto,",
    "  Update" + moduleNameUpper + "Dto,",
    '} from "@/modules/' + moduleNameLower + '/types/' + moduleNameLower + '.types";',
    "",
    ...
    "}",
    "",
    "const " + moduleNameUpper + "Form = forwardRef<" + moduleNameUpper + "FormRef, { onSuccess?: () => void }>(",
    "  ({ onSuccess }, ref) => {",
    ...
    "        if (editingId) {",
    "          await update({ id: editingId, data: values as Update" + moduleNameUpper + "Dto });",
    "        } else {",
  ...
  var tablePath = path.join(pagesComponentsDir, moduleNameUpper + "Table.tsx");
