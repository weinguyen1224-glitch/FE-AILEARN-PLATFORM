...
> {
  ...
  async create(createDto: TCreateDto): Promise<TEntity> {
    try {
      ...
      const result = await request<TEntity>(this.resourcePath, {
        method: 'POST',
        data: createDto,
      });
  ...
  async getPage(options?: QueryOptions): Promise<PageableResponse<TEntity>> {
    try {
      ...
      const result = await request<PageableResponse<TEntity>>(url);
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to get page`,
  ...
  async getMany(options?: QueryOptions): Promise<TEntity[]> {
    try {
      ...
      const result = await request<TEntity[]>(url);
      return result;
    } catch (error) {
      this.logger.error(
        `Failed to get many`,
  ...
  async count(options?: QueryOptions): Promise<number> {
    try {
      ...
      const result = await request<CountResponse>(url);
      return result.count;
    } catch (error) {
      this.logger.error(
        `Failed to count`,
  ...
  async exists(options?: QueryOptions): Promise<boolean> {
    try {
      ...
      const result = await request<ExistsResponse>(url);
  ...
  async findById(id: number | string): Promise<TEntity> {
    try {
      ...
      const result = await request<TEntity>(this.getResourceUrl(`id/${id}`));
      if (!result) {
        throw new ServiceError(
  ...
  async findByMa(ma: string): Promise<TEntity> {
    try {
      ...
      const result = await request<TEntity>(this.getResourceUrl(`ma/${ma}`));
      if (!result) {
        throw new ServiceError(
  ...
  async update(id: number | string, updateDto: TUpdateDto): Promise<TEntity> {
    try {
      ...
      const result = await request<TEntity>(this.getResourceUrl(String(id)), {
        method: 'PUT',
        data: updateDto,
      });
  ...
  async updateMany(
    ...
    try {
      ...
      const result = await request<AffectedResponse>(
        this.getResourceUrl('update-many'),
        {
          method: 'PUT',
          data: reqData,
        },
  ...
  async deleteMany(filter: Record<string, unknown>): Promise<AffectedResponse> {
    try {
      ...
      const result = await request<AffectedResponse>(
        this.getResourceUrl('delete-many'),
        { method: 'DELETE', data: reqData },
      );
  ...
  async softDelete(filter: Record<string, unknown>): Promise<AffectedResponse> {
    try {
      ...
      const result = await request<AffectedResponse>(
        this.getResourceUrl('soft-delete'),
    ...
    try {
      ...
      for (let i = 0; i < createDtos.length; i += BATCH_SIZE) {
        ...
        const batchPromises = batch.map((dto) =>
          request<TEntity>(this.resourcePath, {
            method: 'POST',
            data: dto,
          }),
        );
